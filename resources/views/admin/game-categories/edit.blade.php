...
<div class="form-container">
    ...
    <div class="form-card">
        <form method="POST" action="{{ route('admin.game-categories.update', $gameCategory) }}">
            @csrf
            @method('PUT')
            ...
            <div class="form-group-custom">
                ...
                <select class="form-select-custom" name="game_id" required>
                    ...
                    @foreach($games as $game)
                        <option value="{{ $game->id }}" {{ old('game_id', $gameCategory->game_id) == $game->id ? 'selected' : '' }}>
                            {{ $game->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group-custom">
                ...
                <div class="price-input-wrapper">
                    <input 
                        ...
                        name="price" 
                        value="{{ old('price', $gameCategory->price) }}" 
                        step="0.01" 
                        min="0" 
                        placeholder="0.00"
                        required
                ...
                <div class="info-box">
                    ℹ️ السعر الحالي: {{ number_format($gameCategory->price, 2) }} ريال
                </div>
                @error('price')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            ...
            <div class="form-group-custom">
                <label class="checkbox-wrapper">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $gameCategory->is_active) ? 'checked' : '' }}>
                    <span class="checkbox-label">✓ تفعيل هذه الفئة</span>
                </label>
            </div>

            <!-- Actions -->
            <div class="form-actions">
                <button type="submit" class="btn-submit">💾 تحديث الفئة</button>
