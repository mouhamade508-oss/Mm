                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="applies_to" name="applies_to" required 
                                class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition">
                            <option value="all" {{ old('applies_to', 'all') === 'all' ? 'selected' : '' }}>كل الأقسام</option>
                            <option value="products" {{ old('applies_to') === 'products' ? 'selected' : '' }}>قسم المنتجات</option>
                            <option value="game_recharge" {{ old('applies_to') === 'game_recharge' ? 'selected' : '' }}>قسم شحن الألعاب</option>
                        </select>
                        @error('applies_to')
                            <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                        @enderror
