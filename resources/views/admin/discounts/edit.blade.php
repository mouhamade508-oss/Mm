...
@section('page-title', 'تعديل كود الخصم')
...
<div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 px-4 py-8">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-4xl font-bold text-gray-900 mb-2">✏️ تعديل كود الخصم</h1>
            <p class="text-gray-600">قم بتحديث بيانات الكود أدناه</p>
        </div>

        <!-- Code Info -->
        <div class="mb-6 bg-orange-50 border-l-4 border-orange-500 p-4 rounded-lg flex items-start gap-3">
            <span class="text-2xl">🎁</span>
            <div>
                <p class="font-semibold text-orange-800">معلومة</p>
                <p class="text-orange-700 text-sm">تعديل: <strong class="font-mono text-lg">{{ $discount->code }}</strong> • عدد الاستخدام: <strong>{{ $discount->used_count }} / {{ $discount->usage_limit }}</strong></p>
            </div>
        </div>
        ...
        <!-- Form Card -->
        <form method="POST" action="{{ route('admin.discounts.update', $discount) }}" class="bg-white rounded-lg shadow-lg overflow-hidden p-8 space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="pb-8 border-b">
                ...
                <div class="mb-6">
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition">{{ old('description', $discount->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="pb-8 border-b">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" id="percentage" name="percentage" value="{{ old('percentage', $discount->percentage) }}" required 
                        ...
                        <div class="mt-2 p-3 bg-orange-50 rounded-lg">
                            <p class="text-sm text-orange-800"><strong>معاينة:</strong> <span id="preview-discount">{{ $discount->percentage }}</span>% خصم</p>
                        </div>
                        @error('percentage')
                            <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                        @enderror
                ...
                <div id="product_field" class="{{ old('type', $discount->type) === 'specific' ? '' : 'hidden' }} mt-6">
            ...
            <div class="pb-8 border-b">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" id="valid_from" name="valid_from" value="{{ old('valid_from', $discount->valid_from->format('Y-m-d')) }}" required 
                               class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition">
                        @error('valid_from')
                            <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                        @enderror
            ...
            <div class="pb-8 border-b">
                ...
                <div>
                    ...
                    <input type="number" id="usage_limit" name="usage_limit" value="{{ old('usage_limit', $discount->usage_limit) }}" required 
                           min="1"
                           class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition">
                    ...
                    <div class="mt-4 p-4 bg-blue-50 rounded-lg border-2 border-blue-200">
                        <div class="grid grid-cols-3 gap-4 text-center">
                            <div>
                                <p class="text-sm text-gray-600 font-semibold">مستخدم</p>
                                <p class="text-2xl font-bold text-blue-600">{{ $discount->used_count }}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-600 font-semibold">المتبقي</p>
                                <p class="text-2xl font-bold text-purple-600">{{ $discount->usage_limit - $discount->used_count }}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-600 font-semibold">النسبة</p>
                                <p class="text-2xl font-bold text-orange-600">{{ number_format(($discount->used_count / $discount->usage_limit) * 100, 0) }}%</p>
                            </div>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-3 mt-4">
                            <div class="bg-gradient-to-r from-blue-500 to-blue-600 h-3 rounded-full" style="width: {{ min(($discount->used_count / $discount->usage_limit) * 100, 100) }}%"></div>
                        </div>
                    </div>
            ...
            <div class="pb-8">
                ...
                <div class="flex items-center gap-3 p-4 bg-green-50 rounded-lg border-2 border-green-200">
                    <input type="checkbox" id="is_active" name="is_active" value="1" 
                           {{ old('is_active', $discount->is_active) ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 cursor-pointer">
