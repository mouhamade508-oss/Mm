...
<div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 px-4 py-8">
    <div class="max-w-3xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-4xl font-bold text-gray-900 mb-2">✏️ تعديل الفئة</h1>
            <p class="text-gray-600">قم بتحديث معلومات الفئة</p>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            <form method="POST" action="{{ route('admin.categories.update', $category->id) }}" class="p-8">
                @csrf
                @method('PUT')
                ...
                <!-- Slug (Read-only) -->
                <div class="mb-6">
                    <label for="slug" class="flex items-center text-gray-700 font-bold mb-3 gap-2">
                        <span class="text-xl">🔗</span> الـ Slug
                    </label>
                    ...
                    <p class="text-sm text-gray-500 mt-2">يتم إنشاؤه تلقائياً من اسم الفئة</p>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea id="description" name="description" rows="4" 
                              ...
                              class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition">{{ old('description', $category->description) }}</textarea>
                    @error('description') 
                        <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-8 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <p class="text-gray-700 mb-2">
                        <span class="font-bold">عدد المنتجات في هذه الفئة:</span>
                        <span class="text-blue-600 font-bold">{{ $category->products()->count() }}</span>
                    </p>
                </div>

                <!-- Submit Buttons -->
                <div class="flex gap-4 pt-6 border-t">
                    <button type="submit" class="flex-1 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-bold py-3 px-6 rounded-lg transition transform hover:scale-105">
                        ✅ تحديث الفئة
                    </button>
